...
public class RegisterForEvent extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (inValidName || inValidStartDate || inValidEndDate || inValidSTime || inValidETime || inValidnumOfPeople) {
			...
			// Selects a set of tables that exsist in the event that have at
			// most 4 more
			...
			try {
				...
				for (int counter = tables.size() - 1; counter >= 0; counter--) {
					System.out.println("This is the amount of seatsPerTable for each itteration of the table Array: "
							+ tables.get(counter).getSeatsPerTable());
			...
			//
			// String sql = "SELECT tableAmount FROM tables WHERE id =" +
			// optimalTable.getId();
			// c = DriverManager.getConnection(url, SQLuser, SQLpass);
			// ResultSet rs = c.createStatement().executeQuery(sql);
			//
