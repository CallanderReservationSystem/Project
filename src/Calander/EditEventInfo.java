...
public class EditEventInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String SQLuser = "cs3337stu03";
		String SQLpass = "K!c7YAg.";
		String sqlThrow = "update events set title = '" + title + "', end_date = '" + end_date + "', start_date = '"
				+ start_date + "', start = '" + start + "', end = '" + end + "', tableCount = '" + tableCount
				+ "', seatsPerTable = '" + seatsPerTable + "', location = '" + location + "' where id = " + eventID;
		try {
			c = DriverManager.getConnection(url, SQLuser, SQLpass);
			Statement st = c.createStatement();
